...
func init() {
	android.RegisterModuleType("saturn_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = saturnExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = saturnExpandVariables(ctx, dep_file)
	...
	exCmd := saturnExpandVariables(ctx, String(g.properties.Cmd))
